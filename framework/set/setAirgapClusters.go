...
func AirgapClusters(client *rancher.Client, terraformConfig *config.TerraformConfig, terratestConfig *config.TerratestConfig,
	...
	if !isWindows {
		newFile, file, err = airgap.SetAirgapRKE2K3s(terraformConfig, terratestConfig, newFile, rootBody, file)
		if err != nil {
	...
	if isWindows {
		newFile, file, err = airgap.SetAirgapRKE2Windows(terraformConfig, terratestConfig, newFile, rootBody, file)
		if err != nil {
			return newFile, file, err
		}
	}
